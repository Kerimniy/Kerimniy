...
for (let url of urls) {
  ...
  fetch(`${url}?time=${String(Date.now())}`, {
    method: "get",
    cache: "no-store"
  }).then(r => {
    return r.json()
    ...
    let r = createMonitorRaw(data1, fetched, start)
    fetched = r[1]
    monitor_raw = r[0]

    monitors_raw.push(monitor_raw)


    if (fetched == urls.length) {
      init_monitors(monitors_raw)
      splash = document.getElementsByClassName("splash")
...
function createMonitorRaw(data1, fetched, start) {

  let ping = (performance.now() - start) / 1000

  let data = JSON.parse(atob(data1.content))

  let monitor_raw = { "name": data.name, "group": data.group }
  ...
  let blocks_from_last = Math.max(Math.floor((unixTime - data.checks[data.last_changed]) / data.interval) - 1, 0)
  if (data.checks[data.last_changed] == 0) {
    blocks_from_last = 0
  }
  let c = blocks_from_last
  for (let i = 0; i < Math.min(blocks_from_last, 30); i++) {

    checks.push({ "timestamp": unixTime - (data.interval * (i + 1)), "ok": 0 })
  }

  for (let i = data.last_changed; i > -1; i--) {
    ...
    let unixTime = Math.floor(Date.now() / 1000);
    let is_ok = (unixTime - data.checks[i]) <= (data.real_interval * 33)
    if (is_ok) {
      checks.push({ "timestamp": data.checks[i], "ok": 1 })
    }
    else {
      checks.push({ "timestamp": data.checks[i], "ok": 0 })
    }

    let missing_count = Math.floor((data.checks.at(i) - data.checks.at(i - 1)) / data.real_interval) - 1
    if (data.checks.at(i - 1) == 0) {
      missing_count = 0
    }

    let lasttime = data.checks[i]
    for (let j = 0; j < missing_count; j++) {
      lasttime -= data.real_interval
      checks.push({ "timestamp": lasttime, "ok": 0 })
  ...
  for (let i = 29; i > data.last_changed; i--) {
  ...
  monitor_raw.checks = checks
  monitor_raw.uptime = uptime



  fetched++

  return [monitor_raw, fetched]
}


function init_monitors(monitors_raw) {
    ...
    for (let service of monitors[group]) {
      ...
      for (let i = 0; i < 30 - service["checks"].length; i++) {
        ...
        check_el.addEventListener("mouseenter", (e) => {
          ...
          const width = tool_tip.offsetWidth;

          tool_tip.style.top = (e.clientY + 15) + "px";
          tool_tip.style.left = (e.clientX - width) + "px";
        })
